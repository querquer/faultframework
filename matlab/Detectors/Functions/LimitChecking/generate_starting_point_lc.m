...
% This function returns a starting point for an optimization-algorithm. Hence, 
...
%% Integer values
% The used function for feature extraction and the window size are specified using integer
...
% The lower bound of this range can be defined here. The threshold is not
...
%% Upper bounds
% In order to specify the upper bound of each parameter, we can define
% the array 'UB' like we did define LB before. The threshold should not be limited to
...
% please see function <generate_starting_point_nn.html generate_starting_point_nn>.
ConstraintFunction = [];
end
